...
public class BasicEncryptedDbOperationScenario extends Scenario {


    public BasicEncryptedDbOperationScenario() {
        super("Realm encrypted write 1000 read 1000");
    }

    @Override
    public long runScenario() {
        long time = System.nanoTime();
        Realm realm = RealmSingleton.instance().getRealmSecureInstance();
        realm.beginTransaction();
        realm.delete(UserRealm.class);
        for(int i=0;i<1000;i++) {
            UserRealm user = realm.createObject(UserRealm.class);
            user.setEmail("amailu@gmail.com");
